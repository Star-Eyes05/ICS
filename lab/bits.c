...
int countTrailingZero(int x){
    int r16,r8,r4,r2,r1,r0,ans;
    r16 = (!(((x >> 16) << 16) ^ x)) << 4;  //检查低16位是否全为0
    x = x >> r16;
    r8 = (!(((x >> 8) << 8) ^ x)) << 3;
    x = x >> r8;
    r4 = (!(((x >> 4) << 4) ^ x)) << 2;
...
int divpwr2(int x, int n) {
    return ((~(x >> 31)) & (x >> n)) + ((x >> 31) & ((x + (1 << n) + ~0) >> n));
}
/* 
...
int satMul3(int x) {
  int sign = x >> 31;
  int ans = (x << 1) + x;
  int over = ~((((x << 1) >> 31) ^ sign) | ((ans >> 31) ^ sign)); //溢出则为 0
  int Tmin = 1 << 31;
  int Tmax;
  Tmax = ~Tmin;
  return (over & ans) + (~over & ~(sign ^ 0) & Tmax) + (~over & (sign ^ 0) & Tmin);
}
/* 
...
int isLessOrEqual(int x, int y) {
  return ((!((y + ~x + 1) >> 31)) & ((x >> 31)|~(y >> 31))) | (((x >> 31) & 1) & (~(y >> 31)));
}
/*
 ...
 */
int ilog2(int x) {
  int r16,r8,r4,r2,r1,ans;
  r16 = (!!(x >> 16)) << 4; //检查x/2^16是否为0
  x = x >> r16;
  r8 = (!!(x >> 8)) << 3;
  x = x >> r8;
  r4 = (!!(x >> 4)) << 2;
...
unsigned float_i2f(int x) {
  int tmp = x,musk = 0x7fffff;
  unsigned frac = 0,around = 0,sign = 0,E = 0,exp = 0;
  if(x == (1 << 31))
    return 0xcf000000;
  if(x == 0)
    return 0;
  ...
  around = x & 0xff;
  frac = (x >> 8) & musk;
  frac = frac + ((around > 0x80) || ((around & 0x80) && ((frac & 1) == 1))); // 四舍五入以及向偶数舍入
  if(frac >= (1 << 23))
  {
    frac = frac & musk;
    exp = exp + 1;
...
int float64_f2i(unsigned uf1, unsigned uf2) {
  unsigned sign = 0,exp = 0,frac = 0,ans = 0;
  int E = 0;
  sign = uf2 >> 31;
  exp = (uf2 & 0x7ff00000) >> 20;
  E = exp - 1023;
  frac = ((uf2 & 0xfffff) << 11) + (uf1 >> 21) + (1 << 31);
  if(E > 30)
    return 0x80000000u;
  if(E < 0)
    return 0;
